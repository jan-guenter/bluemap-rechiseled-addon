...
archive_temp="$(mktemp -d /tmp/bluemap-rechiseled-gallery.XXXXXX)"
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/rechiseled-gallery.zip" -@
)
unzip -tq "$archive_temp/rechiseled-gallery.zip"
cp "$archive_temp/rechiseled-gallery.zip" "$output_path"
