...
import io.github.janguenter.bluemap.rechiseled.profile.ExactModArtifactDetector;
import io.github.janguenter.bluemap.rechiseled.profile.ProfileDisablement;
import io.github.janguenter.bluemap.rechiseled.profile.Rechiseled125Fusion1312Profile;
import io.github.janguenter.bluemap.rechiseled.profile.RechiseledDefinition;
...
final class RechiseledResourceExtension implements ResourcePackExtension {

    static final Key SYNTHETIC = Key.parse("bluemap_rechiseled:fusion_model");
    ...
    RechiseledResourceExtension(ResourcePack resourcePack, RechiseledRuntime runtime) {
    ...
    public void loadResources(Iterable<Path> roots) throws IOException, InterruptedException {
        if (ProfileDisablement.current().isDisabled(
                Rechiseled125Fusion1312Profile.PROFILE_ID
        )) {
        ...
        ActiveResourceSchemaValidator.Result schema = ActiveResourceSchemaValidator.validate(
                resourcePack,
                roots,
                Rechiseled125Fusion1312Profile.RESOURCES,
                Rechiseled125Fusion1312Profile.TEXTURES
        );
        if (!schema.valid()) {
    ...
    public Set<Key> collectUsedTextureKeys() {
        ...
        Set<Key> used = new LinkedHashSet<>(
                Rechiseled125Fusion1312Profile.TEXTURES.keys()
        );
    ...
    public Key getBlockStateKey(Key key) {
        return runtime.route().isActive()
                && Rechiseled125Fusion1312Profile.ROUTED_BLOCKS.contains(key.getFormatted())
                ? SYNTHETIC : key;
    }
    ...
    public void getBlockProperties(BlockState state, BlockProperties.Builder builder) {
        ...
        RechiseledDefinition definition = Rechiseled125Fusion1312Profile.DEFINITIONS.get(
    ...
    private Map<Key, Boolean> sheetOpacity() {
        ...
        for (Key source : Rechiseled125Fusion1312Profile.TEXTURES.keys()) {
            TextureCatalog.Entry entry = Rechiseled125Fusion1312Profile.TEXTURES.get(source);
            boolean opaque = true;
    ...
    static boolean reachableTile(TextureCatalog.Entry entry, int index) {
        return !(entry.layout() == io.github.janguenter.bluemap.rechiseled.profile.TextureLayout.FULL
                && index == 41);
    }
    ...
    private Map<TileKey, Key> plannedTiles() {
        ...
        for (Key source : Rechiseled125Fusion1312Profile.TEXTURES.keys()) {
            ...
            int count = entry.layout().columns() * entry.layout().rows();
            for (int index = 0; index < count; index++) {
                Key output = Key.parse("bluemap_rechiseled:tiles/"
    ...
    private Map<Key, Texture> cropTiles(Map<TileKey, Key> planned) throws IOException {
        ...
        for (Map.Entry<TileKey, Key> request : planned.entrySet()) {
            ...
            TextureCatalog.Entry entry = Rechiseled125Fusion1312Profile.TEXTURES.get(tile.source());
