...
/** Hash-locked metadata for one exact routed Rechiseled block. */
public record RechiseledDefinition(
        String blockId,
        ShapeFamily shape,
        int legalStates,
    ...
    public RechiseledDefinition {
        ...
        if (!blockId.startsWith("rechiseled:")
                || legalStates != shape.legalStates()
                || !blockstateSha256.matches("[0-9a-f]{64}")
                || !directModelsSha256.matches("[0-9a-f]{64}")) {
            throw new IllegalArgumentException("malformed Rechiseled rendering definition");
        }
    }
}
