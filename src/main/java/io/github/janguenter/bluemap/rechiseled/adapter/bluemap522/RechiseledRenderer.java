...
final class RechiseledRenderer implements BlockRenderer {
    ...
    public void render(
        ...
        if (!runtime.route().isActive() || catalog == null
                || !Rechiseled125Fusion1312Profile.ROUTED_BLOCKS.contains(blockId)) {
        ...
        try {
            boolean success = forEachOriginalVariant(
                    block,
                    target,
                    variant -> emitter.render(block, variant, target, mapColor, catalog)
            );
            if (!success) {
