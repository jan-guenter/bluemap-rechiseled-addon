...
public final class ResourceManifest {
    ...
    public static ResourceManifest load(String resource, int expectedRows, String expectedSha256) {
        return load(resource, expectedRows, expectedSha256, "rechiseled");
    }
    ...
    public static ResourceManifest load(
        ...
        for (String line : new String(raw, StandardCharsets.US_ASCII).split("\n", -1)) {
            ...
            if (!entry.path().startsWith("assets/" + namespace + "/")) {
                throw new IllegalStateException("resource manifest leaves its namespace");
            }
